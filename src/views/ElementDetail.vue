...
<button  id="atom-detail" :class="` text-lg cursor-auto flex-col  min-w-[420px] max-w-[420px]  mt-7  flex  px-7 pb-5 border rounded-md ${shadow} transition-all duration-500`" >
    <div class="flex flex-col" v-if="data=='all'||!isElement">
        ...
        <span class="mt-1 text-left"><span class="font-extrabold">Source:</span> <a class="text-blue-300 font-semibold" href="https://en.wikipedia.org/wiki/Periodic_table">Vikipedia</a></span>
</div>
    <div v-else>

        <div class="text-2xl my-4" :style="style"> {{ data.name }}</div>
        <div v-if="'bohr_model_3d' in data" >
<model-viewer :class="`w-full h-60 rounded-md  transition-all duration-500 ${shadow}`" :src="data.bohr_model_3d" ar ar-modes="webxr scene-viewer quick-look" camera-controls tone-mapping="neutral" poster="poster.webp" shadow-intensity="1" autoplay>
...
<div class="w-full text-left text-lg mt-2 flex flex-col">
    ...
    <span class="mt-1"><span class="font-extrabold ">Discovered by: </span><span  class=""> {{ data.discovered_by }}</span></span>
    <span class="mt-1"><span class="font-extrabold ">Electron Configuration: </span><span :style="style" class="text-base"> {{ data.electron_configuration_semantic }}</span></span>
    <span class="mt-1"><span class="font-extrabold ">Atomic Number: </span><span :style="style" class="text-base"> {{ data.number }}</span></span>
    <span class="mt-1"><span class="font-extrabold ">Atomic Mass: </span><span :style="style" class="text-base"> {{ data.atomic_mass }}</span></span>
